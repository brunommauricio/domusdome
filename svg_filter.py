def clean_svg(svg_name):# feed the svg full name with extension as an argument
    ...
    for i in range(0, len(svg)):
        if svg[i] == "<path":#spot the paths location 
            ...
            svg[i] = svg[i][5:-1].split(' ')# remove the first 5 and the last item of the line of each path, put each vector into a iterable list
    ...
    for n in range(0, len(vector_paths)):
        ...
        for m in range(0, len(vector_paths[n])):
            vector_paths[n][m] = float(vector_paths[n][m])
...
print  clean_svg("domusdomezones.svg")
